...
namespace CizaTable.Editor
{
	public class GoogleSpreadsheetLoaderEditorWindow : EditorWindow
	{
		...
		private GoogleSpreadsheetLoader Config
		{
			...
			{
				_config = value;
				var guid = _config is null ? string.Empty : AssetDatabase.AssetPathToGUID(AssetDatabase.GetAssetPath(_config));
				ConfigGuid = guid;
			}
		}
		...
		private void GoogleSpreadsheetLoaderArea()
		{
			...
			if (_googleSpreadsheetLoaderEditor is null)
				_googleSpreadsheetLoaderEditor = UnityEditor.Editor.CreateEditor(Config, typeof(GoogleSpreadsheetLoaderEditor));
